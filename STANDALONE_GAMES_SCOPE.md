...
> ## CORRECTION — 2026-09-02, after this report was written
>
> Two figures quoted below as unconfirmed doc claims have since been **measured
> against prod** (`nezhuwyfirrbmyojpiyx`) during the Supabase load
> investigation, and both are false:
>
> | Claim | Source | Prod, measured |
> |---|---|---|
> | "Standalone games (~40% of prod)" | `CLAUDE.md` #20 | **0 of 40 games** have `competition_id IS NULL` |
> | "18 of 23 prod games" are complete + scoring + never-published | `CLAUDE.md` #25 | **1 of 40** |
>
> §8.1 below leaned on the first one, and the passage is corrected in place.
> **The finding §8.1 reports is unaffected and, if anything, stronger** — the
> gap is that no surface lists a competition-free game, which is true whether
> the count is 40% or zero. What changed is the severity: it is a latent gap in
> the shipped app rather than one currently biting 40% of production data.
>
> The second figure is quoted in §7 only as an example of a stale doc claim, so
> nothing here rests on it. Both are filed together — the rule each supports is
> still correct; only the supporting count has aged out.
...
So a competition-free game is reachable **only by URL** today.

**This paragraph originally leaned on CLAUDE.md #20's "~40% of prod", and that
figure is false.** Measured 2026-09-02: `SELECT count(*) FROM games WHERE
competition_id IS NULL` returns **0**, out of 40 games. So no production game is
currently stranded by this — the gap is real and latent, not currently biting.

That makes it *cheaper* to fix and *less urgent*, which is a different call from
the one the 40% figure would have produced. Worth knowing before anyone
prioritises it off this report.
...
4. ~~**Prod figures are unconfirmed.**~~ **Now measured, and both are false** —
   see the correction at the top. CLAUDE.md #20's "~40% of prod games are
   standalone" is **0 of 40**; #25's "18 of 23 prod games" in the
   complete+scoring+never-published state is **1 of 40**. Neither rule is wrong;
   both supporting counts have aged out. Flagging that this entry did its job:
   it was labelled unconfirmed, and the label is what made it cheap to go and
   check rather than expensive to discover later.
