...
describe("getTripCountdown", () => {
  ...
  it("returns idea when isIdea is true (regardless of dates)", () => {
    expect(getTripCountdown("2026-06-01", "2026-06-05", true)).toEqual({ type: "idea" });
    expect(getTripCountdown(null, null, true)).toEqual({ type: "idea" });
  });

  it("returns no_dates when start or end is missing (not an idea)", () => {
    expect(getTripCountdown(null, null, false)).toEqual({ type: "no_dates" });
    expect(getTripCountdown("2026-06-01", null, false)).toEqual({ type: "no_dates" });
    expect(getTripCountdown(null, "2026-06-05", false)).toEqual({ type: "no_dates" });
  });

  it("returns weeks when more than 14 days away", () => {
    // 2026-04-26 → 2026-12-31 ≈ 36 weeks
    const result = getTripCountdown("2026-12-31", "2027-01-07", false);
  ...
  it("uses singular 'week' for 1 week (edge case at 7 days exactly is days, not weeks)", () => {
    ...
    const result = getTripCountdown("2026-05-26", "2026-05-30", false);
    expect(result.type).toBe("weeks");
    if (result.type === "weeks") {
  ...
  it("returns days when 2-14 days away", () => {
    ...
    const result = getTripCountdown("2026-05-05", "2026-05-08", false);
  ...
  it("returns 'Tomorrow' when daysUntil is 1", () => {
    const result = getTripCountdown("2026-04-27", "2026-04-30", false);
    expect(result).toEqual(
      expect.objectContaining({
        type: "days",
        days: 1,
  ...
  it("returns 'Today is the day' when daysUntil is 0", () => {
    const result = getTripCountdown("2026-04-26", "2026-04-30", false);
    expect(result).toEqual({
      type: "today",
  ...
  it("returns happening when today is between start and end (inclusive)", () => {
    ...
    const result = getTripCountdown("2026-04-24", "2026-04-28", false);
    expect(result).toEqual({
      type: "happening",
      dayNumber: 3,
      totalDays: 5,
  ...
  it("returns happening on the end day itself", () => {
    ...
    const result = getTripCountdown("2026-04-22", "2026-04-26", false);
    expect(result).toEqual({
      type: "happening",
      dayNumber: 5,
      totalDays: 5,
  ...
  it("returns 'Just wrapped' when ended within 1 week ago", () => {
    ...
    const result = getTripCountdown("2026-04-19", "2026-04-22", false);
  ...
  it("returns 'N weeks ago' when ended 2-8 weeks ago", () => {
    ...
    const result = getTripCountdown("2026-03-01", "2026-03-05", false);
    expect(result.type).toBe("past");
    if (result.type === "past") {
  ...
  it("returns past_distant ('Month YYYY') when more than 8 weeks ago", () => {
    ...
    const result = getTripCountdown("2025-12-01", "2025-12-05", false);
    expect(result).toEqual({
      type: "past_distant",
      label: "December 2025",
    });
