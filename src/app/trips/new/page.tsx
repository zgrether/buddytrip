...
export default function TripNewPage() {
  ...
  const handleCreate = async (withDestination: boolean) => {
    setError("");
      ...
      {step === 2 && (
        <main className="mx-auto max-w-2xl space-y-6 px-4 pb-16 pt-6">
          ...
          <div>
            ...
            >
              <MapPin size={16} style={{ color: "var(--color-bt-text-dim)", flexShrink: 0 }} />
              <input
                ...
                onKeyDown={(e) =>
                  e.key === "Enter" && destination.trim() && handleCreate(true)
                }
          ...
          {/* Skip */}
          <button
            data-testid="skip-destination-btn"
            onClick={() => handleCreate(false)}
            disabled={isSubmitting}
            className="w-full py-2 text-sm transition-opacity disabled:opacity-40"
            style={{ color: "var(--color-bt-text-dim)" }}
          >
            That&apos;s okay, fill it in later
          </button>
        </main>
      )}
    </div>
