    ...
    <div className="space-y-3">
      {expenses.length === 0 ? (
          ...
          action={
            canEdit && !showAdd ? (
              <button
                ...
                className="flex items-center gap-2 rounded-xl border px-4 py-2 text-sm transition-colors hover:bg-[var(--color-bt-hover)]"
        ...
        <>
          ...
          {/* Add expense button (non-empty list) */}
          {canEdit && !showAdd && (
            <button
              data-testid="show-add-expense-btn"
              onClick={() => setShowAdd(true)}
...
export function ExpensesTab({ trip, canEdit }: TabProps) {
