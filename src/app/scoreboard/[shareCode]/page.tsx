...
export default function PublicScoreboardPage() {
  ...
  if (error || !data) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-3" style={{ background: "#0d1117", color: "#e6edf3" }}>
        <Trophy size={32} style={{ color: "#30363d" }} />
  ...
  return (
    ...
    >
      ...
      <header className="flex items-center gap-3 px-4 py-4">
        <Trophy size={20} style={{ color: "#00d4aa" }} />
      ...
      <div className="px-4 space-y-5">
            ...
            {teamScores.map((ts) => {
              ...
              return (
                <div key={ts.teamId} className="flex-1 text-center">
                  ...
                  <p className="text-3xl font-bold" style={{ color: team?.color ?? "#e6edf3" }}>
                    {ts.totalPoints}
                  </p>
                </div>
              );
          ...
          {totalPoints > 0 && (
            <div className="mt-3 flex h-2 overflow-hidden rounded-full" style={{ background: "#30363d" }}>
              {teamScores.map((ts) => {
                ...
                return (
                  <div key={ts.teamId} style={{ width: `${pct}%`, background: team?.color ?? "#8b949e" }} />
                );
              })}
            </div>
          )}
        ...
        {/* Rounds table */}
        <section>
          <h2 className="mb-3 text-xs font-semibold uppercase tracking-wider" style={{ color: "#8b949e" }}>
            Rounds
          </h2>
          <div className="overflow-hidden rounded-xl" style={{ background: "#161b22", border: "1px solid #30363d" }}>
            <div
              ...
              {sideEvents.map((se) => (
                ...
                >
                  <div className="flex items-center gap-2">
                    <span>{se.icon}</span>
                    <span className="text-sm" style={{ color: "#e6edf3" }}>{se.name}</span>
                  </div>
                  <span
        ...
        <p className="pt-4 text-center text-xs" style={{ color: "#6e7681" }}>
          Powered by BuddyTrip
        </p>
      </div>
    </div>
