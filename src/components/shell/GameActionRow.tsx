...
function ActionRow({ chrome }: { chrome: NonNullable<ReturnType<typeof useGameChrome>> }) {
    ...
    <div
       ...
       */
      /**
       * NO negative margin here — see the panel's own class in
       * `CompetitionFace`, which is where the flush-under-the-bar offset lives.
       *
       * #938 put `lg:-mt-6` on THIS element and it clipped the row: the panel is
       * `overflow-y-auto`, and a first child pulled above a scroll container's
       * origin is not reachable by scrolling — it is simply cut off. The
       * measurement that "verified" it read `getBoundingClientRect().top`, which
       * reports the geometric position of a clipped box perfectly happily. The
       * offset belongs on the panel BOX, which nothing clips.
       */
