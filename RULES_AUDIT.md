...
|---|---|---|
| ✅ **"That colour is hardcoded in 17+ places — every instance must be migrated"** | It's in **two** places now | **Fixed** — re-derived, not renumbered |
| ✅ **The style guide's clean-up list** | Three named files no longer exist; two whole patterns were already finished; one entry was **wrong**, not just stale (it pointed white-on-danger at a token that is near-black-on-teal, contradicting §5 of the same document); and it **missed new instances** of a pattern it did list. The "no migration needed" team-colour list had drifted the other way — naming 4 of 8 colours and 6 dim variants that aren't in the palette, so it would have flagged half the real palette as violations | **Fixed** — the per-file checklist is replaced by a re-derivation command plus the two genuinely open items, and exceptions are named by the module they live in rather than by value |
...
| ✅ **The description of one notification category** | It said the category "defaults OFF and carries an in-context bell toggle." It defaults **ON**, and the bell **was removed** — with the correct explanation ten lines above it in the same file | **Fixed**, and it had **four** instances, not one. Three in notification code (the category description, the file header naming the removed bell as a consumer, and a test comment reasoning from the wrong default — the test itself was sound). The fourth was outside notification code entirely, so no notification-scoped sweep would have found it: a comment in the user menu pointing at "the notifications bell / trip switcher panels in TopNav" as its reference implementation. Neither exists there any more. That one is a different *kind* of decay — not a false claim but a **dangling pointer**, and the lesson it carries is that naming a specific sibling to copy is what rots; describing the behaviour doesn't |
