-- 082 — save_game_config: a DISABLE now writes config too (draft-then-save P1.8)
--
-- 081 shipped the true→false branch as disable-ONLY: it flipped scoring_enabled,
-- reverted the match rows, and RETURNed before any config was written. That made the
-- settings page's live-lock unfixable from the client. The toggle reads the DRAFT, so
-- staging "Setup" looks like it unlocks the rows — but `settingsEditable` had to keep
-- reading the SERVER's flag, because any edits riding the same payload as the disable
-- were silently discarded by that early RETURN. The result was a clunky two-step
-- (Save the disable → rows unlock → edit → Save again), and unlocking the rows
-- without this change would have converted it into silent data loss.
--
-- Now the branch falls THROUGH: config is written first (under the existing course /
-- matches freeze guards), then the disable happens at the end — symmetric with the
-- go-live block. One atomic Save both disables and re-configures.
--
-- Consequence, accepted deliberately: a disable KEEPS scores, so "flip to Setup +
-- change the matchups + Save" on a scored game is REFUSED (HAS_SCORES) and the whole
-- tx rolls back — including the disable. That's correct: the two-step couldn't have
-- done it either (the second Save would have hit the same guard), and a partial apply
-- would be worse than an honest refusal. The user resets the scores, or drops the
-- match edits and saves the disable alone.
--
-- The RAISE messages are also rewritten to name the ACTUAL affordance ("Reset scores"
-- in the game's Danger zone) rather than restate the error code — the tRPC front door
-- maps these to user-facing copy, and its fallback leaks the raw text verbatim, so a
-- raw 'HAS_SCORES' can reach the Save banner.
--
-- Everything else is byte-identical to 081. 081 is APPLIED, so it is never edited —
-- this is a new migration (CLAUDE.md Migration Workflow).
...
BEGIN
  ...
  -- true→true is the ONLY rejected transition: a live game's settings are
  -- client-locked, so a save claiming to keep it live is a stale page. Everything
  -- else — false→false, false→true, and (082) true→false — writes config below.
  ...
  IF v_matches_dirty THEN
    ...
    -- tx (disable included) rolls back. Deliberate — see the header.
    SELECT EXISTS (SELECT 1 FROM public.score_entries WHERE game_id = p_game_id)
        OR EXISTS (SELECT 1 FROM public.match_hole_outcomes WHERE game_id = p_game_id)
      INTO v_has_scores;
    IF v_has_scores THEN
