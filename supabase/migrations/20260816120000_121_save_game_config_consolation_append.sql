-- ════════════════════════════════════════════════════════════════════════════
-- 121 · A consolation match can be ADDED after play has started
--
-- Reported: turning on the 3rd-place match mid-bracket produced
--   "Save failed: HAS_PICKS: this bracket already has results. Reopen it for
--    corrections in the game's Danger zone before changing the field or the draw."
--
-- The refusal was wrong, not merely badly worded.
--
-- ── The guard and the operation were never the same size ───────────────────
-- `v_bracket_dirty` is a symmetric set difference between the incoming draw and
-- the stored one on (bracket, round, slot, a_seed, b_seed). Enabling consolation
-- ADDS one row, so `want EXCEPT ALL have` is non-empty, so the draw is "dirty",
-- so HAS_PICKS fires. An APPEND was indistinguishable from a REBUILD.
--
-- And the remedy behind the guard is an unconditional clean-replace (DELETE
-- every match and entrant, re-insert), so even past the guard the picks would
-- have been destroyed. Narrowing the check alone would have been worse than the
-- refusal: it would have let the write through.
--
-- Same shape as #896, where `matches.addMatch` turned out to be a pure additive
-- insert behind a blanket refusal.
--
-- ── What this changes ──────────────────────────────────────────────────────
-- When the ONLY difference between the incoming draw and the stored one is
-- CONSOLATION rows, the write is additive: insert the added ones, delete the
-- removed ones, and touch nothing else. HAS_PICKS is not consulted, because
-- nothing it protects is at risk — the main draw's rows, and therefore every
-- recorded winner in them, are not read or written on this path.
--
-- A consolation match is contested by the two semi-final LOSERS and its
-- occupants are derived at read time (112's advancement model), so the appended
-- row carries null seats and needs no entrant changes. `buildDraw` already
-- refuses to emit one below two rounds, and `bracketPlacements` already splits
-- the tied 3rd when it is decided.
--
-- ── What is still refused ──────────────────────────────────────────────────
-- Any difference touching the MAIN draw: a changed field, a reseed, a different
-- entrant count. Those rewrite round-1 seats, which is exactly what makes a
-- recorded pick meaningless, and they still take the clean-replace path behind
-- HAS_PICKS. The enumeration is the point — the guard now matches the size of
-- the operation instead of the table it happens to touch.
--
-- ── Turning it OFF is allowed here, and confirmed in the client ────────────
-- Removing a consolation match that has a recorded winner destroys a result.
-- That is a real loss, but it is the user's to make — the same shape as every
-- other destructive-but-legitimate action in this app, which confirms in the
-- client and is permitted by the server (ClearPairingsPrompt, DiscardChanges).
-- A server refusal would leave no way to undo an accidental toggle at all.
--
-- Replaced verbatim from migration 117 with the consolation path added.
-- ════════════════════════════════════════════════════════════════════════════
