-- 158 — a game delegate may set what their game is worth.
--
-- ══ Revisiting a Slice D decision, in the open ═════════════════════════════
--
-- `points_total` has been trip-staff-only since Slice D (#360, cd3935c6),
-- where `games.d_addgame.test.ts` pinned it by name — "a delegate can
-- distribute to the total but CANNOT change the total" — and that file's header
-- states the model: "owner sets the total; a game-delegate distributes within".
--
-- That decision is three months old and is revised here deliberately, with its
-- test updated to say so rather than deleted.
--
-- **The original rationale supports the change.** The reason given was that the
-- Owner sets the total because they understand which games are being played and
-- how they should be weighted. That is a statement about who TYPICALLY does it,
-- not about who should be PERMITTED to — and the permission was derived from
-- the habit.
--
-- Pick'em makes it concrete. A delegate builds the slate, opens picks, locks
-- them and enters the results; the total is a primary control on the settings
-- page they own. Making them ask the Owner to change one number, on a role
-- PERMISSIONS.md defines as "an Owner scoped to one game", is the wrong shape.
--
-- The sibling column already agreed: `points_distribution` — the other half of
-- the same settings row — has always been in the delegate tier. The split was
-- the odd thing, not the widening.
--
-- ══ The silent drop, which is a separate defect and is fixed here too ══════
--
-- A delegate is ADMITTED to this function by `assert_game_edit`, and both
-- `points_total` and `delegates` were gated AGAIN inside it on `v_is_org`.
-- The effect was not a refusal: their save SUCCEEDED, every other column
-- persisted, and the gated column was dropped with no error. The page reports
-- saved and the value is gone.
--
-- Widening `points_total` removes one instance. The MECHANISM is fixed for
-- `delegates`, which keeps its staff gate: a non-staff caller who sends a
-- CHANGED delegate set is now refused outright. An UNCHANGED set still passes,
-- because every format's payload builder may include the key, and refusing on
-- presence would stop a delegate saving anything at all.
--
-- `delegates` keeps the gate on purpose: granting delegation is "changes who
-- is trusted", the one thing the tier above the delegate exists to hold — a
-- delegate still cannot sub-delegate.
--
-- ══ Ordering ══════════════════════════════════════════════════════════════
--
-- **This migration REPLACES `save_game_config` whole and its body is derived
-- from the 157 version**, including 157's pick'em freeze blocks. It must be
-- applied AFTER 157; applying it to a database without 157 fails on the
-- missing `_pickem_has_results`, which is the correct way for that mistake to
-- surface.
