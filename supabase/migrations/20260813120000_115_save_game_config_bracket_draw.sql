-- ════════════════════════════════════════════════════════════════════════════
-- 115 · save_game_config writes the bracket POOL and DRAW
--
-- Phase 2b. 112 created the tables, 113 taught the atomic save the bracket's
-- SCALAR settings; this adds the two list-shaped slices — `bracket_entrants`
-- (+ their members) and `bracket_matches` — so the whole bracket setup page
-- commits through ONE call like every other format (CLAUDE.md #18).
--
-- ── Deterministic entrant ids, and why they are not a cute trick ────────────
-- An entrant's id is `<game_id>:e<seed>` rather than a fresh uuid. That is load-
-- bearing for the config hash, not a convenience.
--
-- `bracket_matches.entrant_a_id` is the semantic content of a draw — it IS "who
-- is in this slot" — so #16 requires hashing it. With minted uuids, every write
-- of an UNCHANGED draw would produce different ids and churn the fingerprint,
-- which is the exact trap `game_delegates.granted_by` fell into. A deterministic
-- id is a pure function of the game and the seed, so an identical draw hashes
-- identically, and the id encodes exactly the fact the hash needs.
--
-- The alternative — hashing seeds instead of ids — needs a join in the hash read
-- and two FKs from `bracket_matches` to the same table to disambiguate. More
-- machinery in the one place that must not be subtly wrong.
--
-- Match ids stay minted: nothing references them, so they carry no meaning and
-- are excluded from the hash as re-minted identity.
--
-- ── The DIRTY check is computed HERE, not sent by the client ────────────────
-- `matches`/`groups` take a `*StructureDirty` flag from the client, defaulting
-- TRUE for conservatism — a terse client gets a rebuild rather than a silent
-- skip. That default is safe there and DESTRUCTIVE here: the settings page
-- re-sends its whole draft on every save, so a rename on a bracket mid-play
-- would rebuild the draw and delete every pick made so far. Defaulting the other
-- way is worse (a real change silently ignored).
--
-- So there is no flag. The RPC compares the payload's pool and draw to what is
-- stored and rebuilds only on a real difference. A no-op save writes NOTHING,
-- which is what makes the hash stable without a client having to be careful.
--
-- ── HAS_PICKS — the destroys tier for a bracket ────────────────────────────
-- Reshaping the draw once results exist is the bracket's version of re-pairing a
-- scored match: the rebuild deletes match rows, and a recorded winner goes with
-- them. Refused with the same shape and the same voice as HAS_SCORES, naming the
-- affordance rather than restating the condition.
--
-- Note it cannot key on `score_entries` — a bracket has none. Its result lives
-- in `bracket_matches.winner_entrant_id`, so `v_has_scores` is permanently FALSE
-- for a bracket and every existing guard is inert on it. Same trap migration 111
-- documented for finalized NON-GOLF games, arriving for the same reason.
--
-- ── What this deliberately does NOT write ──────────────────────────────────
-- `winner_entrant_id`. Setup builds the tree; picking who won is a separate act
-- with a separate permission and its own undo (the correction flow). It lands in
-- phase 3. A rebuild here leaves existing winners alone because a rebuild cannot
-- happen while any exist.
--
-- Replaced verbatim from migration 113 with the new block (step 2d) added.
-- ════════════════════════════════════════════════════════════════════════════
