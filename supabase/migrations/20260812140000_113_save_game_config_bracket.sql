-- ════════════════════════════════════════════════════════════════════════════
-- 113 · save_game_config writes games.bracket_config
--
-- Phase 2a of the bracket format. 112 created the schema; this teaches the ONE
-- atomic save to persist the bracket's SCALAR settings — elimination
-- (single/double), entrants (singles/partners), seeding mode, consolation.
--
-- The POOL and the DRAW (bracket_entrants / bracket_matches) are NOT here. They
-- are list-shaped and clean-replaced, like `groups` and `matches` above them, and
-- they land in their own migration once their payload shape is settled. Splitting
-- them keeps this change to one line of write logic against a function that is
-- already 500 lines of guard.
--
-- ── Why COALESCE-PRESERVE and not the unconditional overwrite ──────────────
-- Every format sends the FULL config it owns, and a key it does not own is
-- absent → that column resets to the honest empty. That rule is right for
-- `modifiers` (a golf game genuinely owns its modifier set) and WRONG here for
-- the same reason it is wrong for `competition_format`: only a bracket game
-- sends `bracketConfig`, so an absent key means "not mine to speak for", not
-- "clear it". Without the COALESCE, any other format saving its own settings
-- would wipe a bracket's setup.
--
-- ── Already hashed, deliberately ────────────────────────────────────────────
-- `bracket_config` joined GAME_CONFIG_COLS in 112, BEFORE anything wrote it, so
-- the cross-device sync invariant ("everything the RPC writes must be hashed")
-- is satisfied by the time the write exists rather than after someone notices.
-- That ordering is the point: four fields went silent by being hashed late.
--
-- Replaced verbatim from migration 111 with one field added to the scalar UPDATE.
-- ════════════════════════════════════════════════════════════════════════════
