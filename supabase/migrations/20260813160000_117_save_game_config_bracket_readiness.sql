-- ════════════════════════════════════════════════════════════════════════════
-- 117 · A bracket needs a FIELD before it can go live
--
-- Phase 2c made Bracket selectable and shipped its setup page. Nothing yet
-- required the setup to have happened: `scoring_enabled` could be flipped on a
-- bracket with an empty field, and the crew would arrive at a game with no draw.
-- This is the server half of #917.
--
-- ── Why this could not be a client-only check ──────────────────────────────
-- The obvious fix is two lines in `NonGolfGameView`'s `management.ready`. It was
-- deliberately NOT done that way. Go-live readiness is re-asserted here, inside
-- the transaction, precisely so the client gate cannot be bypassed — and a
-- refusal the client makes and the server does not is the same divergence in the
-- other direction: two answers to one question, agreeing only by luck. So the
-- server learns the rule first (this migration, landing alone per the Migration
-- Workflow), and the client's copy follows in the PR that reads it.
--
-- ── Placement: the ELSE arm, and why it reads POST-write ───────────────────
-- The non-golf arm, alongside the existing "points are configured" check. Every
-- readiness check in this block runs LAST for one reason, stated at step 4 and
-- worth restating because the bracket is the newest thing to depend on it: the
-- pool is written by step 2d of THIS SAME CALL. A settings page that builds a
-- field and switches the game to Scoring in one Save must be judged on the field
-- it is establishing, not the one it is replacing — reading pre-write state here
-- would refuse the ordinary first setup, with no save order that would work.
--
-- That is the same mistake #918 fixed at the tRPC front door for the place
-- ceiling. It is recorded here because this block is where the codebase already
-- got it right (093's points check reads `points_total` FRESH for exactly this
-- reason), and the next check added here should inherit that rather than
-- rediscover it.
--
-- ── Two entrants, not one ──────────────────────────────────────────────────
-- A one-entrant draw has no match to play; `buildDraw` produces nothing to show.
-- Two is the smallest field with a game in it. This matches `bracketPlaceCapacity`
-- (#916), which returns a null ceiling below two entrants for the same reason —
-- there is no draw yet, so the pool is still being built.
--
-- ── Scoped to `competition_format = 'bracket'`, NOT the legacy values ──────
-- `bracket_se` / `bracket_de` are read-accepted but never offered (they predate
-- the collapse to one Bracket entry). They also predate migration 112, so no such
-- game has `bracket_entrants` rows at all — gating them would make an existing
-- live legacy bracket unsaveable, failing on a field that never existed. Verified
-- against production before writing this: there are currently ZERO games with any
-- bracket format, legacy or current, so nothing is affected either way; the
-- narrow scope is what keeps that true if one appears.
--
-- ── Re-checked on every re-affirm, deliberately ────────────────────────────
-- Unlike 093's zero-points check (`NOT v_was_live`, fresh enable only), this is a
-- STRUCTURAL check and behaves like match play's paired-matches one: it runs
-- whenever `scoringEnabled` is true. So emptying a live bracket's field is
-- refused rather than silently leaving a live game with no draw.
--
-- Replaced verbatim from migration 115 with the new check added.
-- ════════════════════════════════════════════════════════════════════════════
