-- ════════════════════════════════════════════════════════════════════════════
-- 185 · save_game_config learns about skins — three edits, and each is a
--       DIFFERENT class of miss
--
-- Migration 184 added `skins_hole_outcomes`, a seventh shape a game's score can
-- be stored in. This function does not read it, and three separate things in it
-- are wrong as a result. Landed as its own migration rather than folded into 184
-- so the diff has a reader: the function is ~1100 lines and is replaced
-- wholesale, so the only way to review it is to be told what changed.
--
-- ── 1 · `v_has_scores` — a played skins game read as scoreless ─────────────
--
-- Every locked-tier guard in this function keys on one boolean, computed once
-- from `score_entries` OR `match_hole_outcomes`. A skins game has rows in
-- neither, so a fully played one satisfied none of them: its course could be
-- changed under it, and so on. This is CLAUDE.md #27's shape — a "has this been
-- played?" predicate blind to a storage shape it was written before.
--
-- ── 2 · the groupings freeze, which is NEW behaviour and not a widening ────
--
-- Explained at the site. The short version: section 2b re-mints every
-- `play_groups` id on any structural groups save, and skins is the first format
-- whose score hangs off those ids rather than off the players in them. Edit 1
-- alone would not have caught it — the guard beneath it asks whether a SCORED
-- PLAYER was dropped, and a skins rename drops nobody while still destroying
-- every recorded hole.
--
-- That is worth stating plainly, because "add the table to `v_has_scores`" looks
-- like the whole fix and is not. The two guards answer different questions and
-- only one of them was ever asked about groups.
--
-- ── 3 · the go-live arm — 182's lesson, applied instead of re-learned ──────
--
-- The readiness gate has three arms: match play needs every match paired;
-- stroke, rack and scramble need at least one GROUPED participant; everything
-- else needs a point value. Scramble reached prod in the third arm and had to be
-- moved by 182, which recorded the general form — "enumerate the GUARDS, not
-- only the writers", since a procedure that gates on a column without writing it
-- is invisible to a survey of what writes it.
--
-- Skins belongs in the second arm for a stronger reason than scramble does. A
-- member's right to record a hole resolves through
-- `game_participants.play_group_id` (`can_score_skins_grouping`, 184), and the
-- grouping is also the carryover boundary — so a groupless skins game has nobody
-- who may enter anything and no pot to enter it into. Going live would produce a
-- board that cannot be filled in.
--
-- ── How this file was produced ─────────────────────────────────────────────
--
-- The body is 182's, GENERATED from that file rather than transcribed, with the
-- three edits applied programmatically and the canonical/spliced pair diffed to
-- prove they are the only changes. A hand copy of 1100 lines is a transcription
-- risk that no reviewer can see; the first attempt at 184's much smaller
-- `_reset_game_scoring` copy silently reproduced the wrong migration's body and
-- dropped #895's go-live triple, which is the argument for doing it this way.
-- ════════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  IF v_type = 'gtt_match_play' THEN
    ...
    IF (v_gfh_was_on, v_gfh_was_n) IS DISTINCT FROM (v_gfh_now_on, v_gfh_now_n) THEN
      -- The FURTHEST-ALONG hole in the game, across every match. Outcome rows are
      -- the only source that matters: `gloriousConfig` returns NO_GLORIOUS for
      -- score entry, and the guard above refuses that combination outright, so a
      -- score-mode game can never reach here with GFH on either side.
      SELECT max(hole_number) INTO v_gfh_max_played
      FROM public.match_hole_outcomes WHERE game_id = p_game_id;
