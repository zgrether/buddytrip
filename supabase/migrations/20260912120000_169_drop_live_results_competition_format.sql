...
-- two are separate files (and separate PRs), and why CI cannot exercise this.
--
-- Confirmed before this was opened, against production and by reading the DATA
-- rather than `schema_migrations` — a recorded migration version says a file
-- ran, not that a row moved:
--
--   live_results : 0 rows          (was 1)
--   head_to_head : 3 rows          (was 2 — the repointed game arriving)
--
-- and the game itself intact: still `complete`, still 5 points, still its two
-- `game_results` rows. Re-run the count immediately before applying this
-- anyway; the gap between opening a PR and pushing is not zero.
--
...
-- here — and that is now MEASURED, not predicted: zero rows hold
-- `live_results`, so nothing re-sends it, so there is nothing to keep it
-- accepted FOR. The code stopped offering and accepting it in #1168, which has
-- merged and deployed; this is the second half of CLAUDE.md 3b's removal
-- ordering, not the first.
--
