-- 157 — pick'em's scoring settings get ONE freeze point: the first result.
--
-- ══ What this changes, and why the old boundary could not work ═════════════
--
-- Pick'em's settings page had FOUR write models on one screen:
--
--   * Total Points        — wrote to the server on every stepper press
--   * Deadline            — a private draft with its own commit button
--   * Roll-up / confidence — a private draft with its own Save button
--   * Name / Rules / Delegates — wired to nothing at all; typing Rules of the
--     Day and closing the panel lost it silently
--
-- Every other format's settings page is one draft: change things, nothing
-- persists, Cancel or Save. The machinery for that (`useConfigDraft`, the
-- frozen baseline, the dirty check, the save bar) has existed since #626 and
-- pick'em simply never adopted it.
--
-- What blocked the adoption was not the UI. It was that pick'em's three
-- scoring settings had TWO different freeze boundaries:
--
--   * `roll_up` / `use_confidence` froze when picks OPENED (148)
--   * `points_total` was deliberately carved OUT of that freeze (152),
--     because freezing it would strand a 0-point game mid-trip
--
-- One atomic save cannot honour two boundaries. It would be refused whole once
-- picks opened, taking `points_total` down with it — which is exactly the
-- state 152 was written to prevent.
--
-- ══ The boundary that dissolves it ═════════════════════════════════════════
--
-- All three move to the FIRST RESULT. Until something has been scored,
-- changing how scoring works rewrites nothing — there is no result to
-- invalidate, and every sheet is re-read through whatever the rules are.
--
-- That is one boundary for one page, so one atomic save is expressible, and
-- 152's carve-out becomes unnecessary rather than merely weaker.
--
-- The slate keeps its OWN boundary — picks-open — and that is correct: people
-- are choosing against those games right now, and adding or removing one
-- invalidates every ranking. Two guards, two questions, no longer conflated.
--
-- ══ The one thing that gets more permissive, stated plainly ════════════════
--
-- `use_confidence` can now be turned ON after picks are locked. Everyone who
-- already submitted gets slate-order defaults they did not choose.
--
-- That is recoverable and consistent — it is exactly the sheet a non-submitter
-- has, and `reconcileSheet` raises `rankingReset` so the screen SAYS the
-- ranking is not theirs rather than presenting it as a choice they made. The UI
-- warns before the save. It is a real consequence, not a hidden one.
--
-- ══ A behaviour change worth declaring ═════════════════════════════════════
--
-- `points_total` written through `save_game_config` is ORGANIZER-only
-- (`v_is_org`), where `set_pickem_points_total` allowed any game editor,
-- delegates included. So a delegate loses the ability to set a pick'em game's
-- points total. That aligns pick'em with every other format — 152's own comment
-- notes "every other format sets its total through save_game_config's draft" —
-- but it IS a narrowing, and it is deliberate rather than incidental.
--
-- ══ What is NOT done here ══════════════════════════════════════════════════
--
-- `set_pickem_points_total` is not dropped. The client still calls it until the
-- draft adoption lands, and CLAUDE.md's rule 3b puts the code change first and
-- the drop in a follow-up. It gains the freeze here so it cannot be a hole
-- through the boundary in the meantime.
--
-- `HASH_COLS` gains `pickem_games` in the client change, not here — it is
-- TypeScript. Until it does, pick'em settings are written atomically but are
-- not yet fingerprinted for cross-device sync.

-- ── The one definition of "has this game been scored" ───────────────────────
--
-- Container fact, not a caller fact: its answer does not depend on who asks, so
-- per CLAUDE.md #28 it is REVOKEd from anon and authenticated and is only ever
-- called from SECURITY DEFINER bodies.
--
-- Conservative by construction. `game_results` and a decided `game_matches`
-- row are each sufficient, and so is a finished game — a freeze that guesses
-- wrong should guess towards refusing.
...
SET search_path TO ''
AS $$
  -- SCOPED TO PICK'EM, in the body, so the name is true of the answer.
  --
  -- Caught by `games.saveConfig.finalized.test.ts`: without this the freeze in
  -- `save_game_config` applied to EVERY format, and a finalized match-play game
  -- could no longer have its points edited — a documented behaviour ("POINTS are
  -- editable after finishing — they recompute at read") broken by a predicate
  -- whose name said pick'em and whose body said any game.
  --
  -- Scoping here rather than at the call site is deliberate: the call site is
  -- where it was forgotten once already, and every other caller
  -- (`save_pickem_config`, `save_pickem_matches`) is pick'em-only anyway, so
...
REVOKE ALL ON FUNCTION public._pickem_has_results(text) FROM PUBLIC;
REVOKE ALL ON FUNCTION public._pickem_has_results(text) FROM anon;
REVOKE ALL ON FUNCTION public._pickem_has_results(text) FROM authenticated;

COMMENT ON FUNCTION public._pickem_has_results(text) IS
  'Has anything been scored on this game yet — a game_results row, a decided game_match, or a finished game? The ONE freeze boundary for pick''em''s three scoring settings (points_total, roll_up, use_confidence), which before migration 157 had two boundaries between them and could not be committed by a single atomic save. A container fact with no auth.uid() in its body, so it is REVOKEd from anon and authenticated per CLAUDE.md #28 and called only from definer bodies.';


-- ── The slate keeps picks-open; the settings move to first-result ──────────
...
-- ── One spelling of the question, shared with the matches guard ────────────
...
BEGIN
  ...
  -- Asks the shared predicate rather than spelling the condition again
  -- (migration 157). It is the same question the settings freeze asks — "has
  -- anything been scored yet" — and two spellings of one question is how the
  -- two drift. Strictly broader than the inline version it replaces, which read
  -- only `game_matches`: a game carrying `game_results` is now refused too,
  -- which is the conservative direction for a guard protecting recorded results.
...
-- ── The legacy points writer cannot be a hole through the new boundary ─────
...
BEGIN
  ...
  -- keeps them apart.
  -- The same boundary the other two scoring settings now use (migration 157).
  -- Without it this function is a hole straight through the freeze: it writes
  -- the same column `save_game_config` refuses to write after a result.
  IF public._pickem_has_results(p_game_id) THEN
    RAISE EXCEPTION 'PICKEM_SCORED: results are in, so how this game scores is frozen'
      USING ERRCODE = 'check_violation';
  END IF;
...
-- ── save_game_config learns pick'em's two settings ─────────────────────────

-- Derived from the DEPLOYED definition (migration 121) with exactly two blocks
-- inserted — the freeze backstop before the config write, and the pickem_games
-- write after the scalar column update. Nothing else is changed; the body was
-- taken from pg_get_functiondef rather than retyped so the untouched 800 lines
-- are byte-identical to what is running.
