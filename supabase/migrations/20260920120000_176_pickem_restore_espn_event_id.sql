-- 176 — save_pickem_config threads espn_event_id through again.
--
-- ══ What happened ═══════════════════════════════════════════════════════════
--
-- 149 added the column and taught save_pickem_config to write it: the INSERT
-- column list and the ON CONFLICT DO UPDATE SET both carried it, and the
-- function's own comment said so ("espn_event_id threaded through in 149").
--
-- 156 and 157 each replaced the function's whole body — plpgsql has no append,
-- CREATE OR REPLACE restates everything — and neither carried it forward.
-- Confirmed by reading both in full: zero mentions of espn_event_id in either
-- file, and no doc (PICKEM-SPEC.md included) records a decision to drop it.
-- 156's own header is otherwise exhaustive — it numbers and explains every
-- intentional change to this function, down to a single COALESCE. Silence on
-- a column three other layers still actively support is the signature of an
-- omission, not a decision: the value simply did not survive the restatement.
--
-- ══ Why this reads as accidental rather than deliberate ════════════════════
--
-- Nothing downstream of 156 stopped supporting the field. The search still
-- emits it (`MatchupSearch.tsx`), the modal still sends it
-- (`PickemSlateModal.tsx:835`), the router still accepts it
-- (`slateGameSchema`, `pickem.ts:64`) and reads it back
-- (`pickem.ts:144,327`). Every layer but the one that got rewritten twice
-- still agrees the field exists. Three call sites doing the work of a fourth
-- that silently stopped is not a redesign.
--
-- ══ The visible consequence ═════════════════════════════════════════════════
--
-- `PickemSlateModal.tsx:314` builds a `taken` set from
-- `draft.map((g) => g.espnEventId)` so the matchup search can grey out a game
-- already on the slate. With the column never written since 156, that set has
-- been empty for anything saved after 2026-08-30 — the search re-offers a
-- game already added, and the runner can duplicate it.
--
-- ══ No backfill ═══════════════════════════════════════════════════════════
--
-- Prod holds 20 slate rows; 1 carries provenance, and it predates 156. The 19
-- written since have no ESPN id to recover — "Alabama at Georgia" plus a
-- kickoff a runner may have edited cannot be reversed into an event id, which
-- is exactly what 149's own header said about this column ("cannot be
-- recovered later from the team names"). New rows populate correctly from
-- here; nothing is done for the 19 already missing it.
--
-- ══ Scope ═════════════════════════════════════════════════════════════════
--
-- Function body only — the column, its comment, and its partial unique index
-- are all still exactly as 149 left them; only the write path needed restoring.
...
BEGIN
  ...
  IF p_payload ? 'slate' THEN
    ...
    LOOP
      ...
      -- espn_event_id restored (migration 176, #1217) -- see this file's
      -- header. Read the same way 149 wrote it: NULLIF on an empty string,
      -- because the router sends '' for a hand-typed row, not null.
...
COMMENT ON FUNCTION public.save_pickem_config(text, jsonb) IS
  'Commits a pick''em settings page atomically. Two freeze boundaries, guarding two different questions (157): the SLATE is frozen while picks are OPEN, the scoring SETTINGS freeze at the first RESULT. An id JOINING the slate costs nothing (174) -- existing ranks stay a legal partial sheet under 166. An id LEAVING is REFUSED (175) when confidence ranks are stored, or when that contest has a result. espn_event_id -- the matchup search''s provenance, and the column the "already on the slate" dedupe reads -- is written on every insert and upsert (149; restored in 176 after 156/157 silently dropped it from two full-body rewrites).';
