-- 087 — save_game_config: drop the `matchesDirty` legacy-key fallback (P3.1 cleanup).
--
-- 084 introduced `v_matches_structure_dirty` as a DUAL-READ —
--   COALESCE((p_payload->>'matchesStructureDirty')::boolean, (p_payload->>'matchesDirty')::boolean, true)
-- — so a stale client still sending the OLD `matchesDirty` key kept working during the
-- deploy window between 084 applying and the new client shipping. That window is now
-- CLOSED: P2 shipped all four formats on the new payload, and no client anywhere emits
-- `matchesDirty` (only match play sends the structure flag, as `matchesStructureDirty`;
-- `matchesDirty` survives solely in code COMMENTS). So the middle term is dead.
--
-- This drops ONLY that legacy term. The gate is unchanged in behaviour:
--   COALESCE((p_payload->>'matchesStructureDirty')::boolean, true)
-- still defaults a MISSING flag to true (conservative rebuild), so a terse client is
-- treated exactly as before — the only removed path is the never-taken `matchesDirty`
-- read. `v_groups_structure_dirty` (085) never had a legacy term, so it's left as-is.
-- Everything else is 086 verbatim (plpgsql CREATE OR REPLACE re-emits the whole body).
--
-- 081–086 applied and immutable; this is a new migration.
