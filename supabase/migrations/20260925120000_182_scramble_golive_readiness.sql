-- ════════════════════════════════════════════════════════════════════════════
-- 182 · Scramble takes stroke's GO-LIVE readiness arm, not the manual one
--
-- Found by building the client half of scramble rather than by reading it:
-- enabling scoring on a fully-configured scramble game was refused with
-- "finish setting up this game before switching it to scoring".
--
-- ── The gate branches by format, and scramble fell through to the wrong arm ──
--
-- `save_game_config`'s go-live check has three arms. Match play needs every
-- match paired. Stroke and rack need at least one GROUPED participant — 089 made
-- groupings mandatory for both, so an ungrouped roster is not a game. Everything
-- else is the manual formats, which need a point value, because a manual game
-- has no other notion of being set up.
--
-- Scramble is stroke play, so it belongs in the second arm. Landing in the third
-- was wrong in BOTH directions, which is why this is a correctness fix rather
-- than a convenience:
--
--   · it REFUSED a correctly configured game, because no point value was set; and
--   · it would have ADMITTED a scramble game with no groups at all.
--
-- The second is the one that matters. For this format the play_group IS the
-- scoring unit, and migration 181 resolves a member's right to write through
-- `game_participants.play_group_id` — so a groupless scramble game has literally
-- nobody who can enter a score. Going live would produce a board that cannot be
-- filled in, which is a worse failure than the refusal that surfaced it.
--
-- ── Why this is a SECOND migration for one feature ──────────────────────────
--
-- 181 enumerated the WRITE path and missed this GUARD. That is CLAUDE.md #25's
-- lesson arriving through a new door — "enumerate the GUARDS, not only the
-- writers" — because a procedure that gates on these columns without writing
-- any of them is invisible to a survey of what writes them. Recorded here rather
-- than folded quietly into the client PR: the schema has to reach prod before
-- the code that depends on it either way, so it earns its own migration.
--
-- ── The same rule is implemented TWICE, and that is not introduced here ─────
--
-- `src/server/lib/gameReadiness.ts` carries this condition again, in TypeScript,
-- as `ROSTER_TYPES`. Both had to learn scramble independently and no test
-- compares them. The duplication predates this feature; scramble is just the
-- first format to prove it is a maintenance cost rather than defence in depth.
-- Filed rather than fixed in passing.
--
-- Body is 179's, with exactly one IN-list widened. Every other line is unchanged
-- and reproduced because the function is replaced wholesale.
-- ════════════════════════════════════════════════════════════════════════════
    ...
    IF v_type = 'gtt_match_play' THEN
      ...
      IF NOT (v_total > 0 AND v_paired = v_total) THEN
        RAISE EXCEPTION 'NOT_READY: finish setting up this game before switching it to scoring'
          USING ERRCODE = 'check_violation';
      END IF;
    ELSIF v_type IN ('gtt_stroke_play', 'gtt_rack_n_stack', 'gtt_scramble') THEN
