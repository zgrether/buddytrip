-- 089 — save_game_config: PRECISE groups guard (removal-only) + stroke grouped-readiness.
--
-- TWO changes; everything else is 088 verbatim (CREATE OR REPLACE re-emits the whole body).
--
-- (1) The groups HAS_SCORES wall was COARSE by choice — it refused ANY groupings change on a
--     scored rack game, mirroring the matches clean-replace refusal "for coherence with match
--     play" (DEFERRED.md §"Rack groupings on a scored game"; #594). But rack/stroke slots are
--     DERIVED, never stored (`computeRack` builds A[k]-vs-B[k] on the fly; only per-team
--     `game_results` persist — `server/lib/rackNStack.ts`), and scores key to `user_id`
--     (`participant_type='user'`), NOT `play_group_id`. So re-grouping / rename / tee-time /
--     GROWTH never orphans a score — the groups rebuild (2b) is non-destructive (INSERT the
--     roster union ON CONFLICT DO NOTHING, never deletes a participant, only reassigns
--     `play_group_id`). The ONE change that strands a scored player is dropping them from every
--     group. This is the precise guard DEFERRED.md designed, now needed by the BBMI late-arrival
--     scenario (add a straggler's cart mid-round → must not be refused).
--
--     Replace the coarse wall with: refuse ONLY when a scored participant is absent from the new
--     groupings (a removal); allow add / re-group / rename / tee-time. Applies to BOTH rack and
--     stroke (same derived-slots, per-player-scores model). Match play is UNTOUCHED — its
--     matchups are authored + STORED in `game_matches`, a real snapshot, so its `HAS_SCORES`
--     matches wall stays.
--
-- (2) Stroke go-live readiness → GROUPED participants only (mirror rack). Groupings are now
--     MANDATORY for stroke (ungrouped = not in the game), so a roster with no groups has nobody
--     to score and isn't ready. The rack branch already filtered `play_group_id IS NOT NULL`;
--     stroke counted ALL participants. Collapse both formats onto the grouped-only count. The
--     tRPC-side `assertGameReady` / `isConfigured` (`server/lib/gameReadiness.ts`) get the same
--     change in lockstep.
--
-- 081–088 applied and immutable; this is a new migration.
...
BEGIN
  ...
  -- disables. Readiness branches per format (match / stroke+rack grouped / else).
  IF v_go_live THEN
