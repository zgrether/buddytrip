-- ════════════════════════════════════════════════════════════════════════════
-- 178 · save_game_config: glorious finishing holes may not revalue a PLAYED hole
--
-- THE RULE: a GFH change is permitted only if it would not alter the weight of
-- any hole that has already been scored. On, off, raising N, lowering N — all
-- four directions, one sentence. Once a hole is played under a given weight,
-- that weight is fixed.
--
-- ── What this is actually fixing ──────────────────────────────────────────────
--
-- Reported: outcomes entered to a decided match with GFH off, then GFH turned
-- on. The scorecard applied it at once; the persisted result did not move —
-- it stayed `complete / a_win / 4&3` when the doubled remaining swing meant the
-- match was no longer over.
--
-- The cause is NOT a missing invalidation (`saveConfig` already recomputes and
-- the client already invalidates). It recomputes with `skipComplete := true`,
-- and a match's `complete` status is ITSELF derived using GFH weighting — so a
-- match that closed out without GFH is frozen against the recompute that would
-- legitimately reopen it. Measured on a 4-up-with-3-to-play fixture:
-- `skipComplete: true` leaves `complete / a_win / 4&3`; `skipComplete: false`
-- gives the correct `active / null / null`.
--
-- Two halves, and this migration is only the first:
--   · HERE — refuse the changes that would rewrite the past.
--   · games.ts — a PERMITTED change can still make a decided match undecided,
--     which the freeze would swallow, so `saveConfig` now passes
--     `skipComplete := false` for a GFH change ONLY. The other five callers
--     (assignPlayer / setHandicap / setPointValue / removeMatch /
--     setParticipantStrokes) keep the freeze: a late INPUT correction should not
--     rewrite a finished match (CLAUDE.md #9). Deliberately untouched, so the
--     flag there does not read as an oversight.
--
-- ── Why the server and not only the settings UI ──────────────────────────────
--
-- The furthest-along hole count is a server-side fact, and a client-only cap is
-- advisory: a direct RPC call or a stale tab holding a pre-change draft would
-- write straight past it. Same reasoning migration 104 gives for the sibling
-- glorious guard, and the same ERRCODE — `object_not_in_prerequisite_state`,
-- deliberately not anything surfacing as a 401, which #689's authExpiry would
-- treat as a dead session and hard-navigate to /login mid-round over a config
-- validation. Each message NAMES the rule (#809) rather than only reporting a
-- refusal.
--
-- ── Nothing in production is affected ────────────────────────────────────────
--
-- No live game holds a result frozen by a GFH flip; this guard is preventive.
-- Existing rows are not repaired and no data moves.
--
-- Body is 172's, with the new guard added after the
-- GLORIOUS_REQUIRES_OUTCOME_ENTRY check and its variables added to DECLARE.
-- ════════════════════════════════════════════════════════════════════════════
