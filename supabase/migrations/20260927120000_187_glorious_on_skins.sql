-- ════════════════════════════════════════════════════════════════════════════
-- 187 · Glorious Finishing Holes on SKINS — the refusal that named an action
--       skins does not have, and the freeze that has to arrive with it
--
-- Enabling glorious on a skins game was refused with:
--
--     "glorious finishing holes doubles a hole's value, which only applies when
--      you record who won each hole. Switch this game to hole-outcome entry, or
--      turn the modifier off."
--
-- Skins DOES record who won each hole. It is the only thing it records — there
-- is no `score_entries` row anywhere in the format. So the refusal was made on a
-- premise the game already satisfied, and it told the reader to switch to a mode
-- the game is effectively already in, using a control skins does not render.
--
-- ── Why the test was wrong when the rule was right ─────────────────────────
--
-- The guard (104, carried forward since) asks `entry_mode = 'score'`. That
-- column is MATCH PLAY's way of asking "do you record who won each hole?" — it
-- was added in 075 for match play's toggle, and match play is the only format
-- that offers one. Skins never sets it, so it sits at the column DEFAULT of
-- `'score'`, and the guard read that default as an answer.
--
-- This is CLAUDE.md's refusal rule, exactly: **a refusal must name an action the
-- reader can take**, and the tell it gives is that following the message cannot
-- clear the condition. It is the same shape as migration 164's `set_pickem_result`
-- telling a points-cup runner to "set the matches first" on a format with no
-- matches.
--
-- ── Why not set `games.entry_mode = 'outcome'` on skins instead ────────────
--
-- That was the first candidate and it is the more tempting one, because the
-- column would then state something TRUE about the format. It was rejected on
-- evidence rather than taste: `gameReadiness.CONFIG_COL_DEPARTED` treats any
-- non-`'score'` entry mode as "somebody configured this", so every skins game
-- would have been born already-touched and `isNew` would report false for a
-- brand-new one — the board's brand-new treatment lost for the whole format, to
-- fix a guard. A column set to make one predicate pass, which then changes a
-- second predicate's answer, is not a tidier fix.
--
-- (`liveProjection` and `computeMatchPlayResults` also branch on the column, and
-- both were checked: each is reached only by match-play formats, so neither
-- would have mis-routed a skins game either way.)
--
-- ── Gated on the TYPE, not exempting skins by name ─────────────────────────
--
-- `IF v_type = 'gtt_match_play'` rather than `AND v_type <> 'gtt_skins'`, so it
-- agrees with the freeze below and so a future format that records hole winners
-- needs no new exception line. Nothing is left unguarded by narrowing it:
-- `game_type_templates.compatible_modifiers` offers `glorious_holes` to match
-- play and skins only, and `gloriousConfig` refuses to weight anything outside
-- match play whatever the modifiers jsonb says.
--
-- ── The FREEZE has to arrive in the same migration ─────────────────────────
--
-- 178 froze glorious against revaluing an already-played hole, and gated it on
-- `v_type = 'gtt_match_play'`. That gap was reported when skins was specced and
-- deliberately not built, because skins could not turn glorious on anyway.
--
-- This migration removes that reason. Shipping the unblock alone would leave a
-- mid-round glorious change free to revalue a hole whose pot has ALREADY BEEN
-- PAID — which is worse here than in match play, where the weight moves a
-- running tally rather than skins somebody has already won. So the two land
-- together; one without the other is a hole this change would be opening.
--
-- The freeze's logic is untouched. Only its SOURCE of played holes widens, to a
-- UNION over both outcome tables — a match game has no skins rows and a skins
-- game has no match rows, so the two cannot both contribute and there is no
-- per-format arm to keep in step.
--
-- ── How this file was produced ─────────────────────────────────────────────
--
-- The body is 185's, GENERATED from that file rather than transcribed, with the
-- edits applied programmatically and the canonical/spliced pair diffed to prove
-- they are the only changes — three hunks, all shown in the PR. Same method 184
-- and 185 used, and for the same reason: a hand copy of 1100 lines is a
-- transcription risk no reviewer can see.
-- ════════════════════════════════════════════════════════════════════════════
...
DECLARE
  ...
  v_status text;
  v_type text;
  -- 178 · glorious finishing holes may not revalue a hole already played.
  v_gfh_prev_mods jsonb;
  v_gfh_next_mods jsonb;
  v_gfh_was_on boolean;
  v_gfh_now_on boolean;
  v_gfh_was_n int;
  v_gfh_now_n int;
  v_gfh_max_played int;
  v_gfh_max_n int;
  v_cur_entry_mode text;
  -- 179 · the scoring type (and its rubric) is fixed once results exist.
  v_cfg_prev jsonb;
  v_cfg_next jsonb;
  v_is_org boolean;
...
BEGIN
  ...
  SELECT EXISTS (SELECT 1 FROM public.score_entries WHERE game_id = p_game_id)
      ...
      -- 184 · a skins game stores its score HERE and in neither table above,
      -- so without this line every locked-tier guard below reads a fully
      -- played skins game as scoreless.
      OR EXISTS (SELECT 1 FROM public.skins_hole_outcomes WHERE game_id = p_game_id)
    INTO v_has_scores;
  ...
  -- ══ SCORING TYPE is fixed once the game starts producing results (179) ═════
  --
  -- Traditional <-> Stableford, and the Stableford RUBRIC with it. See this
  -- migration's header for why the rubric sits INSIDE the lock rather than
  -- beside it, and why the predicate is `game_started` and not this function's
  -- own `v_has_scores`.
  --
  -- The effective NEXT value mirrors the WRITE (COALESCE-PRESERVE), not the
  -- `modifiers` shape: an absent `config` key means LEAVE ALONE, so a save that
  -- never mentions scoring cannot trip this. Same "follow the write, not the
  -- neighbouring guard" reasoning migration 178 sets out at length — two guards
  -- in this function resolve an absent key differently, on purpose.
  IF p_payload ? 'config' THEN
    SELECT g.config INTO v_cfg_prev FROM public.games g WHERE g.id = p_game_id;
    v_cfg_prev := COALESCE(v_cfg_prev, '{}'::jsonb);
    v_cfg_next := COALESCE(NULLIF(p_payload->'config', 'null'::jsonb), '{}'::jsonb);

    -- Compare the SCORING BLOCK only. `config` is a general-purpose column, and
    -- a later setting living beside `scoringType` must not inherit this lock by
    -- accident — a guard that widens silently is how a refusal ends up naming
    -- the wrong object.
    IF (COALESCE(v_cfg_prev->>'scoringType', 'traditional')
          IS DISTINCT FROM COALESCE(v_cfg_next->>'scoringType', 'traditional')
        OR COALESCE(v_cfg_prev->'stableford', '{}'::jsonb)
          IS DISTINCT FROM COALESCE(v_cfg_next->'stableford', '{}'::jsonb))
       AND EXISTS (SELECT 1 FROM public.game_started gs WHERE gs.game_id = p_game_id) THEN
      RAISE EXCEPTION 'SCORING_TYPE_LOCKED: scores are already in, so how this game scores is fixed. Reset scores in the game''s Danger zone to change it.'
        USING ERRCODE = 'object_not_in_prerequisite_state';
    END IF;
  END IF;
  ...
  -- GATED ON MATCH PLAY (187). `entry_mode` is match play's way of asking
  -- "do you record who won each hole?", and it is the only format that has
  -- the column or the toggle. SKINS records a hole winner unconditionally —
  -- there is no second mode and no `score_entries` row anywhere in the format
  -- — but its `entry_mode` sits at the column DEFAULT of 'score', so this
  -- guard fired on every attempt to enable glorious on a skins game and told
  -- the reader to "switch this game to hole-outcome entry": an action skins
  -- has no control for, on a game already in the state being demanded.
  --
  -- That is CLAUDE.md's refusal rule exactly — a refusal that cannot be
  -- cleared by following its own message — and the same shape as migration
  -- 164's pick'em case. The premise was right and the TEST for it was a
  -- match-play proxy.
  --
  -- Gated on the type rather than exempting skins by name so it agrees with
  -- the freeze below, and so a future format that records hole winners needs
  -- no new exception. Nothing is left unguarded: `compatible_modifiers` only
  -- offers `glorious_holes` to match play and skins, and `gloriousConfig`
  -- refuses to weight anything outside match play regardless of the jsonb.
  IF v_type = 'gtt_match_play'
     AND COALESCE(p_payload->>'entryMode', v_cur_entry_mode) = 'score'
     AND COALESCE(
           p_payload->'modifiers',
  ...
  -- ══ GLORIOUS FINISHING HOLES may not revalue a hole ALREADY PLAYED (178) ══
  --
  -- The rule, in one sentence: a GFH change is permitted only if it would not
  -- alter the weight of any hole that has already been scored. That covers all
  -- four directions — on, off, raising N, lowering N. Once a hole is played
  -- under a given weight, that weight is fixed.
  --
  -- Why it is a refusal and not a recompute: people tee off believing they know
  -- the rules. A runner who forgets the switch and notices at the turn can still
  -- fix it, because nothing already played is affected. Retroactively revaluing
  -- a finished hole is the thing that becomes an argument over dinner, in BOTH
  -- directions — somebody who played 16 expecting it to count double should not
  -- have it quietly made normal either.
  --
  ...
  -- ── Sub-18 rounds ─────────────────────────────────────────────────────────
  --
  -- The 18 below is the mechanic's own: `holeWeight` doubles a hole when
  -- `hole > 18 - n` against a literal `ROUND_HOLES = 18` (src/lib/gloriousHoles.ts),
  -- deliberately, so on a shorter round no hole clears the threshold and GFH is
  -- simply inert (DEFERRED.md, "18-N round-length inertia"). The guard uses the
  -- same 18 on purpose: capping against the round's real hole count would refuse
  -- values the mechanic would have honoured, and permit ones it would not.
  -- CONSEQUENCE, recorded here so it is found next to the assumption: on a
  -- sub-18 round this guard permits values of N that do nothing at all. That is
  -- consistent with the documented inertia, and it is the SECOND place holding
  -- the literal 18 — whoever makes the mechanic honour real round length must
  -- change both, and this comment is how they find the second one.
  --
  -- Match play only: `gloriousConfig` is format-guarded, so the key is inert on
  -- any other type even if the jsonb carries it. Refusing there would be a
  -- refusal about a setting that has no effect.
  -- SKINS JOINS THE FREEZE (187), and it must join it in the same change that
  -- lets it turn glorious ON — enabling the modifier without the freeze would
  -- leave a mid-round change free to revalue a hole whose pot has already been
  -- paid, which is a worse failure here than in match play: there the weight
  -- moves a tally, here it moves skins somebody already won.
  IF v_type IN ('gtt_match_play', 'gtt_skins') THEN
    v_gfh_prev_mods := COALESCE((SELECT modifiers FROM public.games WHERE id = p_game_id), '{}'::jsonb);
    -- The WRITE's expression, verbatim — see above.
    v_gfh_next_mods := COALESCE(NULLIF(p_payload->'modifiers', 'null'::jsonb), '{}'::jsonb);
    v_gfh_was_on := v_gfh_prev_mods ? 'glorious_holes';
    v_gfh_now_on := v_gfh_next_mods ? 'glorious_holes';
    -- 3 is GLORIOUS_HOLES_DEFAULT (src/lib/modifiers.ts) — the value the client
    -- writes when the modifier is enabled without an explicit count.
    v_gfh_was_n := CASE WHEN v_gfh_was_on
      THEN COALESCE((v_gfh_prev_mods->'glorious_holes'->>'holes')::int, 3) ELSE 0 END;
    v_gfh_now_n := CASE WHEN v_gfh_now_on
      THEN COALESCE((v_gfh_next_mods->'glorious_holes'->>'holes')::int, 3) ELSE 0 END;

    -- Only judge an actual CHANGE. Re-saving the same setting must stay free —
    -- every other field on the page shares this Save, and a game mid-round would
    -- otherwise be unable to change its name.
    IF (v_gfh_was_on, v_gfh_was_n) IS DISTINCT FROM (v_gfh_now_on, v_gfh_now_n) THEN
      -- The FURTHEST-ALONG hole in the game, across every match or grouping.
      -- Recorded-outcome rows are the only source that matters for either
      -- format: match play reaches here only in outcome mode (the guard above),
      -- and skins has no other shape — it writes no `score_entries` at all.
      --
      -- A UNION rather than a branch on `v_type`: a match game has no skins rows
      -- and a skins game has no match rows, so the two cannot both contribute
      -- and there is no per-format arm to keep in step.
      SELECT max(hole_number) INTO v_gfh_max_played
      FROM (
        SELECT hole_number FROM public.match_hole_outcomes WHERE game_id = p_game_id
        UNION ALL
        SELECT hole_number FROM public.skins_hole_outcomes WHERE game_id = p_game_id
      ) AS played;

      IF v_gfh_max_played IS NOT NULL THEN
        -- (1) ALREADY ON, and a hole inside the CURRENT window has been played.
        --     Frozen completely — not capped. Lowering N de-weights a played
        --     doubled hole; raising N doubles a played normal one; turning it off
        --     de-weights every played glorious hole. No value of N is available,
        --     so the message must NOT offer one: a capped sentence here would name
        --     an action that does not work.
        IF v_gfh_was_on AND v_gfh_max_played > 18 - v_gfh_was_n THEN
          RAISE EXCEPTION 'GLORIOUS_FROZEN: glorious finishing holes can no longer be changed — a group has already played holes under the current setting.'
            USING ERRCODE = 'object_not_in_prerequisite_state';
        END IF;

        -- (2) The round is over. Nothing is left to weight, so there is no N to
        --     offer and "not applicable" is the honest word rather than a cap of 0.
        IF v_gfh_max_played >= 18 THEN
          RAISE EXCEPTION 'GLORIOUS_NOT_APPLICABLE: a group has finished play — glorious finishing holes is not applicable.'
            USING ERRCODE = 'object_not_in_prerequisite_state';
        END IF;

        -- (3) The cap. Anything beyond it reaches back into holes already played.
        --     FURTHEST-ALONG group, not the average and not the trailing one: if
        --     three groups are through 12 and one is through 16, only 2 is honest.
        v_gfh_max_n := 18 - v_gfh_max_played;
        IF v_gfh_now_n > v_gfh_max_n THEN
          RAISE EXCEPTION 'GLORIOUS_WINDOW_PLAYED: a group has already played % holes — you can only use % glorious finishing hole%.',
            v_gfh_max_played, v_gfh_max_n, CASE WHEN v_gfh_max_n = 1 THEN '' ELSE 's' END
            USING ERRCODE = 'object_not_in_prerequisite_state';
        END IF;
      END IF;
    END IF;
  ...
  -- ══ SKINS · a groupings change is refused OUTRIGHT once holes are recorded ══
  --
  -- Not the finer-grained rule below, and the difference is not strictness
  -- for its own sake. Section 2b DELETEs every `play_groups` row and
  -- re-inserts with fresh `gen_random_uuid()` ids, so ANY structural groups
  -- save re-mints the very ids `skins_hole_outcomes.grouping_id` points at —
  -- a rename or a tee-time edit included, since `groupsStructureDirty` covers
  -- those — and the FK cascade takes the recorded holes with them.
  --
  -- For rack and stroke that is harmless: their scores are keyed to the
  -- PLAYER, who survives the rebuild, so the guard below only has to stop a
  -- scored player being DROPPED. Skins is the first format whose score is
  -- keyed to the GROUP, so it is the first that needs the whole set frozen.
  --
  -- Gated on the game TYPE as well as on the rows, which is 090's lesson
  -- rather than belt-and-braces: the EXISTS alone would be a STRUCTURAL
  -- inference ("this game has skins rows, so it must be skins"), and that is
  -- the exact shape 090 had to remove when 089 made stroke groupings
  -- mandatory and a rack-only inference silently widened.
  --
  -- HAS_SCORES prefix so the existing tRPC arm maps it, and the sentence names
  -- the same affordance the other two do. Migration 184 added these rows to
  -- `_reset_game_scoring`, so the action named here can actually clear them —
  -- without that this refusal would send the reader somewhere that does not
  -- work, which is the failure CLAUDE.md's refusal rule is about.
  IF v_type = 'gtt_skins' AND p_payload ? 'groups' AND v_groups_structure_dirty
     AND EXISTS (SELECT 1 FROM public.skins_hole_outcomes WHERE game_id = p_game_id) THEN
    RAISE EXCEPTION 'HAS_SCORES: this game already has holes recorded. Reset scores in the game''s Danger zone before changing its groupings.'
      USING ERRCODE = 'object_not_in_prerequisite_state';
  END IF;
  ...
  UPDATE public.games SET
      ...
      -- `config` (179) — COALESCE-PRESERVE, deliberately NOT the `modifiers`
      -- shape directly above. Only stroke sends this key, so an absent one must
      -- KEEP the value rather than wipe how a whole round scores. Already
      -- hashed (GAME_CONFIG_COLS), so this is the write side catching up.
    ...
    IF v_type = 'gtt_match_play' THEN
      ...
      IF NOT (v_total > 0 AND v_paired = v_total) THEN
        RAISE EXCEPTION 'NOT_READY: finish setting up this game before switching it to scoring'
          USING ERRCODE = 'check_violation';
      END IF;
    ELSIF v_type IN ('gtt_stroke_play', 'gtt_rack_n_stack', 'gtt_scramble', 'gtt_skins') THEN
