-- 085 — save_game_config: carry rack GROUPINGS + standalone PARTICIPANT strokes.
--
-- P2 foundation (rack / stroke / non-golf → draft-then-save). The RPC gains two
-- additive, OPTIONAL payload branches so a non-match format can route its whole
-- config through the same atomic Save. Nothing else changes; match play is untouched
-- (it still sends `matches` and takes the existing path). Backward-compatible: a
-- payload that omits the new keys behaves exactly as under 084.
--
-- WHY the existing RPC couldn't carry these (Phase 0):
--   • `play_groups` were only ever minted inside `_write_game_side` as a 2v2 match
--     container — there was no first-class GROUPINGS write.
--   • the only participant vehicle was `matches[]` (match sides), so a rack/stroke
--     game (participants, no matches) had nowhere to put its roster/strokes.
--
-- (1) GATE THE MATCHES BLOCK on `p_payload ? 'matches'`. Under 084 a payload with no
--     `matchesStructureDirty` key defaulted the flag to TRUE, so a rack/stroke save
--     (no `matches`) would have run the matches clean-replace and DELETEd every
--     game_participant + play_group. The matches guard + write now only fire when the
--     payload actually carries `matches` — match play always does, so it's unchanged.
--
-- (2) GROUPINGS (`groups[]`, rack) — the STRUCTURE unit (membership + name + tee_time).
--     Locked tier: any change on a scored game is refused (issue #594). The coarse
--     wall (whole groupings unit, not just a removal) mirrors the matches clean-replace
--     — a CHOSEN wall for coherence with match play, logged in DEFERRED; the precise
--     boundary (only a scored-participant REMOVAL truly orphans, since rack scores key
--     to user_id not play_group_id) is deferred with the matches upsert-by-identity.
--     When unlocked, it mirrors games.playGroups.setFoursomes: upsert the roster union
--     (keeps existing rows + their scores), rebuild play_groups, reassign play_group_id.
--     Skipped entirely when `groupsStructureDirty` is false → a no-op save leaves
--     play_groups byte-identical.
--
-- (3) PARTICIPANT STROKES (`participants[]`, rack + stroke) — the FIELD (warned) tier:
--     in-place UPDATE of `game_participants.handicap_strokes`, unguarded (a stroke is a
--     recompute INPUT — the tRPC handler re-derives, data-driven on result_strategy,
--     exactly as playGroups.setParticipantStrokes did). Clamp 0–18; 0→NULL to match
--     `_write_side_handicap` so a no-op save round-trips unchanged. Stroke's roster is
--     create-only (P2 ports the model, it doesn't add a roster editor), so stroke never
--     sends `groups` — only this field path on already-created participants.
--
-- Non-golf needs NOTHING here — its points live in the `points_total` /
-- `points_distribution` scalar columns the RPC already writes (084 step 1).
--
-- 081–084 applied and immutable; this is a new migration. Helpers (_write_game_side,
-- _game_side_members, _write_side_handicap) are unchanged and reused.
