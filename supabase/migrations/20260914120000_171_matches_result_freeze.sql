-- ════════════════════════════════════════════════════════════════════════════
-- 171 · save_game_config refuses to re-pair a DECIDED Matches match
--
-- Phase 0 §3 / build spec §5.1 — the freeze gap, and the one the whole handoff
-- called "the careful part". Migration 154's bug through a different door.
--
-- ══ The gap ══════════════════════════════════════════════════════════════
--
-- `v_has_scores` is `score_entries EXISTS OR match_hole_outcomes EXISTS`. A
-- non-golf Matches game (170) has NEITHER — its result is DECLARED straight to
-- `game_matches.result`, never derived from a hole or a gross score — so
-- `v_has_scores` is FALSE for a Matches game with three of four matches
-- decided, exactly the way it is false for a finalized non-golf placement game
-- (the header further down already documents that overlap for a DIFFERENT
-- reason). The existing `HAS_SCORES` guard above is therefore silent on
-- exactly the game that needs a guard: while the game is `active`, re-pairing
-- is PERMITTED, and the structure-dirty branch DELETEs every `game_matches`
-- row and re-mints fresh ids — moving a recorded result onto DIFFERENT PEOPLE,
-- invisibly. Nothing about the resulting row looks wrong afterwards.
--
-- This is pick'em migration 154's guard, needed for a second format that
-- reached the same table the same way. 154's own words say it best: "there is
-- no correct automatic answer to 'you already scored this, and now the people
-- are different'." Refused outright, not merged.
--
-- ══ Why NOT `HAS_SCORES`'s existing message ═════════════════════════════════
--
-- "This game already has scores. Reset scores in the game's Danger zone before
-- changing its matchups" sends the reader to check for scores on a game that
-- has none — CLAUDE.md's own recorded failure mode for a refusal ("a REFUSAL
-- MUST NAME AN ACTION THE READER CAN TAKE", and the companion lesson that a
-- WIDENED condition with an UNCHANGED message is how these happen; migration
-- 162 paid for this exact mistake in pick'em's OWN pairing guard, one migration
-- after 157 made precisely this substitution).
--
-- The ACTION behind the message is real and reachable, which is what makes
-- this fixable with a NEW message rather than a structural fix: `Reset scores`
-- (`GameDangerZone.tsx`) calls `_reset_game_scoring`, and that function's
-- `UPDATE game_matches SET result = NULL, ... WHERE game_id = p_game_id` (162)
-- already clears a Matches game's declared results unconditionally — the same
-- statement that clears a decided golf match. Its own confirmation copy
-- already says "Clears all scores, RESULTS, and bracket picks", so "results"
-- is not a word this migration introduces; it is already on the reader's
-- screen. The new message just names the right OBJECT — a decided MATCH
-- result, not a "score" — while pointing at the SAME button.
--
-- ══ Scope: this check, independent of `v_has_scores` ════════════════════════
--
-- Golf match play is UNAFFECTED in practice: a decided `gtt_match_play` match
-- implies entered scores or recorded hole outcomes, so `v_has_scores` already
-- refuses first and this new check is never reached for it. It is written
-- anyway, generic over every format that reaches this arm, rather than gated
-- on `competition_format = 'matches'` — the condition it tests
-- (`game_matches.result IS NOT NULL`) is what actually matters, not which
-- format produced it, and a second, narrower predicate here would be one more
-- place this fact could drift from the truth.
-- ════════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- 171 — a DECIDED MATCH cannot be re-paired, independent of `v_has_scores`.
  -- See this migration's header for the gap this closes: a non-golf Matches
  ...
  --
  -- The message deliberately does NOT reuse HAS_SCORES's wording. "Reset
  -- scores" sends the reader to check for scores a Matches game does not have
  -- — CLAUDE.md's own recorded failure mode. The ACTION is the same button
  -- (`Reset scores`, whose confirmation already says "Clears all scores,
  -- RESULTS, and bracket picks" — `_reset_game_scoring`'s
  -- `UPDATE game_matches SET result = NULL ...` already clears this); only the
  -- OBJECT named in the message changes, to the thing actually holding the
  -- freeze.
  IF p_payload ? 'matches' AND v_matches_structure_dirty AND EXISTS (
    ...
    RAISE EXCEPTION 'MATCH_DECIDED: a match already has a result. Reset scores in the game''s Danger zone before re-pairing — it clears results too.'
      USING ERRCODE = 'object_not_in_prerequisite_state';
  END IF;
