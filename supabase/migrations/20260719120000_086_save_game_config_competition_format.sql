-- 086 — save_game_config: carry `competition_format` (non-golf's structure label).
--
-- One-line addition to the scalar UPDATE (rest of the function is 085 verbatim, since
-- plpgsql CREATE OR REPLACE re-emits the whole body). Non-golf's competition format
-- (head-to-head / bracket / best-of-N / live-results) was the last setting on its page
-- still self-persisting via a live `games.update` — it couldn't ride the atomic Save
-- because the RPC never wrote the column. Now it does, so P2 phase 2 can make format a
-- draft slice (a live write would move the config hash out from under the frozen
-- baseHash, exactly the reason course had to be drafted for match play).
--
-- COALESCE-PRESERVE (`= COALESCE(p_payload->>'competitionFormat', competition_format)`),
-- NOT the unconditional overwrite the other scalars use: the golf formats never send
-- `competitionFormat`, so an absent key must KEEP the existing value, never null it.
-- Non-golf sends it explicitly to change it. Already hashed (GAME_CONFIG_COLS) — this is
-- purely the write side catching up; Phase 0 §5 missed it (it flagged only the points
-- columns as already-carried). Not a destroys change: no scoring path reads
-- competition_format and the write touches no other rows (Phase 0 §2 → Quiet tier).
--
-- 081–085 applied and immutable; this is a new migration. Helpers unchanged and reused.
