-- ════════════════════════════════════════════════════════════════════════════
-- 179 · save_game_config writes `games.config`, and the SCORING TYPE is fixed
--       once a game starts producing results
--
-- Two changes, one column. `games.config` has existed since the 033 spine and
-- has never been written by anything except the two reset primitives blanking
-- it (125/126). Stableford is the first setting to live there, so this is the
-- write side catching up — the same shape `competition_format` (086) and
-- `bracket_config` (112) took when they became draft slices.
--
-- ── 1 · The write is COALESCE-PRESERVE, not overwrite-with-{} ────────────────
--
-- `modifiers` resolves an absent key to '{}' — a payload that omits it WIPES.
-- `config` deliberately does NOT: absent means LEAVE ALONE, matching
-- `competition_format` and `bracket_config` rather than `modifiers`.
--
-- The reason is the failure mode each shape produces. Four formats share this
-- one RPC and only stroke will send `config`; under the `modifiers` shape every
-- save from a format that does not own the key would silently clear a setting
-- that decides how a whole round scores. That is the "MODIFIERS-MUST-ALWAYS-
-- SEND" hazard (P2) — a rule the client has to remember, enforced nowhere — and
-- the preserve shape removes the need for the rule instead of restating it.
--
-- ── 2 · SCORING_TYPE_LOCKED — the scoring type is fixed before the first score ─
--
-- Switching Traditional↔Stableford mid-round does not merely rescore: the holes
-- already played were PLAYED under different incentives, because under
-- Stableford a blow-up hole stops costing anything past the floor. A card
-- rescored after the fact was never played to the rubric it is being judged by.
--
-- **The RUBRIC is locked by the same guard, not only the type**, and that is
-- deliberate rather than incidental. A floor at +3 and a floor at +2 differ in
-- exactly the property that motivates the lock — where a bad hole stops costing
-- more. Locking the type while leaving the rubric editable would be a half
-- guard whose seam no player could see, so the whole scoring block moves
-- together or not at all.
--
-- Guarded on `public.game_started` (161/170), NOT on this function's own
-- `v_has_scores`. They coincide for a stroke game today — stroke produces
-- `score_entries` and nothing else — but they are different predicates:
-- `v_has_scores` knows two sources, the view knows four and is where a fifth
-- gets added. Keying the lock to the narrower one would make it quietly wrong
-- for the first format that starts some other way.
--
-- The message names the Danger zone because that control exists and clearing
-- scores is what actually lifts the condition — a refusal that names an action
-- the reader cannot take is worse than one that says nothing (CLAUDE.md).
--
-- ERRCODE `object_not_in_prerequisite_state`, matching ENTRY_MODE_LOCKED and
-- the glorious guards, and deliberately nothing that surfaces as a 401 —
-- `authExpiry` would read that as a dead session and hard-navigate to /login
-- mid-round over a config validation (#689).
--
-- ── 3 · A game RESET reverts the scoring type to Traditional. Intended. ──────
--
-- Migrations 125 and 126 already blank `config` to '{}' as part of the reset
-- target state, so a reset game comes back Traditional with no rubric. This is
-- NOT an oversight left un-handled: "Reset game settings" says in its own copy
-- that it clears the whole setup and that only the name and point value
-- survive. Scoring type IS setup. A carve-out keeping it across a reset would
-- contradict what the control promises and would surprise the person who just
-- asked for a clean slate. Recorded here so the next reader finds a decision
-- rather than a gap.
--
-- ── Nothing in production is affected ───────────────────────────────────────
--
-- No row carries a non-empty `games.config` (nothing has ever written one), so
-- the new guard cannot fire on existing data and no rows are repaired or moved.
--
-- Body is 178's, with the `config` column added to the scalar UPDATE, the new
-- guard added after ENTRY_MODE_LOCKED, and its variables added to DECLARE.
-- ════════════════════════════════════════════════════════════════════════════
