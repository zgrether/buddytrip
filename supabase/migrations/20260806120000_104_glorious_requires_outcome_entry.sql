-- 104 — save_game_config: glorious finishing holes requires OUTCOME entry.
--
-- The rule: glorious doubles a hole's value. Outcome entry records who won each
-- hole, so doubling means something. Score entry records a stroke total — you
-- cannot double the value of a hole whose outcome you never recorded. The
-- combination is invalid, and until now nothing enforced it at any layer.
--
-- Why the SERVER and not only the client: the availability gate hides the option in
-- score entry, but a client-only rule is a suggestion. A direct RPC call, a stale
-- tab holding a pre-change draft, or any future caller could still write it.
--
-- Checked on the EFFECTIVE post-save state rather than the payload, because either
-- half can arrive alone: setting glorious on a game that is already score-entry,
-- and flipping a glorious game to score-entry, are the same invalid end state.
--
-- ERRCODE is 'object_not_in_prerequisite_state', matching the sibling guards and
-- deliberately NOT anything surfacing as 401 — #689's authExpiry treats a 401 as a
-- dead session and hard-navigates to /login, which would log someone out mid-round
-- over a config validation. The message NAMES the rule (#809) rather than only
-- reporting that something was refused.
--
-- The four production games already holding this combination are NOT repaired here;
-- they keep their stored values. This guard only reaches them if someone re-saves
-- their config, at which point refusing is correct. Their COMPUTE path is guarded
-- separately in `gloriousConfig`, so a recompute now produces the right result.
--
-- Body is 102's, with the new guard added after the ENTRY_MODE_LOCKED check.
