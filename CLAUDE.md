...
| A slate game that paid nobody | **Void / Voided**, one word everywhere | ONE DB value (`pickem_slate_games.result = 'cancelled'`) and now ONE display word: **Void** where 52px will not hold more (the results-entry segment, the head-to-head swing cell), **Voided** everywhere else (that screen's ENTERED pill, the head-to-head result chip). `push` still needs no such treatment: nothing voided, the game happened and nobody covered. **This REVERSES the earlier per-screen split, and it is the PREMISE that changed rather than the taste.** The split was **Cancelled** where the GAME was the subject and **Void** where the STAKE was — correct while a runner pressing Void was the only producer of the value, because they were asserting the contest did not happen, so "Cancelled — never played" was a fact the row could support. Finalizing with contests outstanding is a SECOND producer of the same value, and those games were probably PLAYED with nobody entering a result — so the game-subject fact is no longer knowable from the value, and only the stake-subject fact survives. One surviving fact does not need two names. Display-string tier; no DB value moved. The old decision rule (*what is this label about*) is what produced the new answer, so it stands — the question that had been asked twice (R3 #1132, R4 #1133) was really "which screen is this on", and the one that lasts is **what can this label still claim**. |
