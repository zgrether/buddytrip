...
| A slate game that paid nobody | **Cancelled** where the GAME is the subject · **Void** where the STAKE is | ONE DB value (`pickem_slate_games.result = 'cancelled'`), two display strings, and the split is the SUBJECT rather than the surface. "The game was cancelled, so the stake voided" — both sentences are true and they are about different things. So the results-entry segment is **Void** (it sets what happens to the stake, and 52px will not hold the longer word) while that screen's ENTERED pill is **Cancelled — never played**; the head-to-head swing cell is **Void** and its result chip is **Cancelled**. Decide by asking *what is this label about*, never by which screen it is on. `push` needs no such split: nothing voided, the game happened and nobody covered. Settled after the question arrived twice (R3 #1132, R4 #1133) and was answered per-screen both times, which is how a second name becomes permanent. |
