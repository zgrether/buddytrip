...
| A slate game that paid nobody | **Cancelled**, one word everywhere | ONE DB value (`pickem_slate_games.result = 'cancelled'`) and ONE display word, now matching it: **Cancelled** on the results-entry segment, that screen's settled status line, the head-to-head swing cell and its result chip. `push` still needs no such treatment: nothing cancelled, the game happened and nobody covered. **THIS IS THE THIRD ANSWER HERE, and the history is the useful part — each step was right on its own terms and each was overtaken from OUTSIDE the argument it was having.** (1) **Cancelled / Void** split by SUBJECT — `Cancelled` where the GAME was the subject, `Void` where the STAKE was. Correct while a runner pressing the button was the only producer of the value, since they were asserting the contest did not happen. (2) **Void / Voided**, when finalizing with contests outstanding became a SECOND producer: those games were probably PLAYED with nobody entering a result, so the game-subject fact stopped being knowable and only the stake-subject one survived — one surviving fact not needing two names. The `Void`/`Voided` pair that replaced it was a WIDTH split, not a meaning one (`Void` where 52px would not hold more). (3) **Cancelled**, everywhere — on a ground neither of the first two weighed: **who is reading it**. Both earlier rounds argued about what the label is ABOUT; this one is about the crew, who are not database users and to whom `Voided` reads as form-processing jargon. The small loss of precision (it hints the game did not happen, which is only sometimes true) costs less than a word half the readers must translate. The width constraint that forced the abbreviation is gone too — the results segment is full-width since the four outcomes stopped sharing a row. Display-string tier every time; no DB value has ever moved. **Deliberately reverses R3 #1132 and R4 #1133** (Zach's call, PR for the pick'em five-surface redesign). The standing lesson is not the word but the shape: a naming rule can be applied correctly and still be answering the wrong question, so record which question a rename was settling — `what is this about` twice, then `who reads it`. |
